...
struct my_double divide_my_double(struct my_double a, struct my_double b)
{
	...
	if (is_zero(b)) return gen_inf(sign);
...
void hack()
{
	while (TRUE)
	{
		printf("enter two double variables to get the results of \"+-*/\" (press CTRL + Z to quit):\n");
		double a, b;
		scanf("%lf %lf", &a, &b);
		...
		add_test(my_a, my_b);
		subtract_test(my_a, my_b);
		multiply_test(my_a, my_b);
		divide_test(my_a, my_b);
	}
}
